...
export const getDashi = async ({ userName, latitude, longitude, date, time, hours, minutes, greenwich }: HoroscopeData) => {
  const { data: vim } = await authRequest.post(`${process.env.REACT_APP_API_URL}/horoscope/get-dashi/`, {
    name_user: userName,
    longitude,
    latitude,
  ...
  const { data: chr } = await authRequest.post(`${process.env.REACT_APP_API_URL}/horoscope/get-dashi/`, {
    name_user: userName,
    longitude,
    latitude,
    dt: date.split('.').reverse().join('-') + 'T' + time,
