...
export const getCharaDashi = async ({ horoscopeData, dateStart }: GetDashiProps): Promise<DashiReturnType> => {
  const { userName, longitude, latitude, date, time, greenwich, hours, minutes } = horoscopeData;

  const { data } = await authRequest.post(`${process.env.REACT_APP_API_URL}/horoscope/get-dashi/`, {
    name_user: userName,
    longitude,
    latitude,
